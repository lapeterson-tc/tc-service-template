...
class JsonDBDAO(Generic[M]):
    ...
    def __init__(self, db: JsonDB, model: type[M]) -> None:
        ...
        self.db = db
        self.model = model
        self.index_field = self.db._get_index_field(model)  # noqa: SLF001
    ...
    def get_all_with_total_count(  # noqa: C901
        ...
        if (
            ...
            if isinstance(sort_by, SortBy):
                ...
                paths = [
                    p
                    for p in paths
                    if filter_fn(self.db._get_index_from_path(p))  # noqa: SLF001
                ]
                data = [
                    self.db.load_from_path(self.model, path)
                    for path in paths[offset : limit + offset if limit else limit]
            ...
            # pylint: disable=protected-access
            paths = [p for p in paths if filter_fn(self.db._get_index_from_path(p))]  # noqa: SLF001
            data = [
                self.db.load_from_path(self.model, path)
                for path in paths[offset : limit + offset if limit else limit]
            ]
        ...
        match where:
            ...
            case None:
                where_val = None
            case _:
                ex_msg = f'Invalid where value: {where}'
